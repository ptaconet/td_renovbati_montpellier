---
title: "TD SIG niveau intermédiaire - étude de cas : priorisation des zones pour la rénovation énergétique du bâti dans l'agglomération Montpellier Méditerranée Métropole"
author: | 
  | Paul Taconet
date: "30/10/2022"
output: 
...
Vous produirez d'abord une carte sur l'étendue spatiale de MMM entière. Vous calculerez l'indice de priorisation de renovation énergétique sur toute la MMM à une résolution spatiale de 200 m (i.e. 1 valeur de l'indice pour chaque carreau de 200 m x 200 m). 

Dans un second temps, vous produirez une carte sur l'étendue d'un quartier de la ville de Montpellier, séléctionné pertinemment grâce à la carte produite à l'étape précédente. L'objectif ici sera de descendre en résolution spatiale par rapport à l'analyse précédente : calculer l'indice de priorisation de renovation énergétique à l'échelle de l'habitat (immeuble ou maison individuelle).
...
- Q2.2 : Raster d'imperméabilité des sols : Que représentent ces données (en une phrase) ? Comment ont été générées ces données (i.e. quelles sont les données source ayant permis de les générer) ? Quelle est la couverture spatiale des données ? La résolution spatiale du raaster ? Quelle est l'étendue des valeurs du raster ? Quelle est le taux d'imperméabilité des sols au niveau du théatre de la Vignette de l'UPVM (R=79) ?  https://land.copernicus.eu/user-corner/technical-library/imperviousness-2018-user-manual.pdf
...
\emotrophy \textit{Avez-vous réussi ?} : 
...
Dans la formule de l'IPRE proposée précédemment, 5 des 6 indicateurs composant la formule doivent être calculés avec la base de données Filosofi : `perc_ind` (% d'individus habitant dans le carreau par rapport au nombre total d'individus dans la MMM), `perc_sup64` (% d'individus de plus de 64 ans dans le carreau), `perc_pauv` (% de ménages pauvres dans le carreau), `perc_inf1990` (% de logements contruits avant 1990 dans le carreau), `perc_coll` (% de ménages en logements collectifs dans le carreau). Chacun de ces indicateurs doivent être calculés à partir d'attributs de la table attributaire.
...
```{r, out.width="1\\linewidth", fig.cap="Paramétrisation de la calculatrice raster pour générer le raster représentant l'IPRE", fig.align="center"}
...
Le travail effectué dans l'analyse n°1 nous a permis de produire une carte des zones à prioriser à une échelle satisfaisante (200 m x 200 m), mais dans une perspective opérationnelle, nous devons prioriser à une échelle plus fine. Dans la suite de ce travail, nous allons descendre en échelle : nous allons séléctionner un quartier de Montpellier et produire une carte de l'IPRE à l'échelle du batiment. Autrement dit, nous allons calculer une valeur d'IPRE pour chaque bâtiment du quartier.
...
\emotrophy \textit{Avez-vous réussi ?} : Q7. Quel est le type de données du jeu de données de distribution fine de la population ? Quelles données sources et quels traitements géographiques ont été utilisés pour le générer ?
